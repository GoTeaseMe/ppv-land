...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
	({ variant = 'secondary', size = 'md', isActive = false, children, className = '', ...props }, ref) => {
		...
		const variantStyles = {
			primary: ['bg-gradient-to-br from-[var(--accent)] to-[#9b7bff]', 'text-[#0b0c10]', 'hover:brightness-105'],
			secondary: [
		...
		const activeStyles = isActive
			? [
					...
					'shadow-[inset_0_0_0_2px_rgba(124,92,255,0.18)]',
				]
			: [];

		const buttonStyles = [...baseStyles, ...sizeStyles[size], ...variantStyles[variant], ...activeStyles, className]
			.filter(Boolean)
			.join(' ');
